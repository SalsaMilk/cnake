...
button button_new(int x, int y, const char* text, BYTE colorR, BYTE colorG, BYTE colorB, BYTE colorA) {
    ...
    _button.textRect.y = _button.rect.y + (_button.textRect.h / 2);
    _button.textRect.x = _button.rect.x - (_button.textRect.w / 2);
    _button.rect.w = _button.textRect.w + _button.textRect.h;
    _button.rect.h = _button.textRect.h * 2;
    _button.rect.x -= _button.rect.w / 2;
    return _button;
}

// react on mouse click within button rectangle by setting 'pressed'
BYTE button_pressed(button *btn, const SDL_Event *ev) {
...
BYTE button_draw(SDL_Renderer *r, button *btn) {
